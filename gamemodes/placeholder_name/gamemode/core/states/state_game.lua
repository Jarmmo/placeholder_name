...
local function drawf(time)
	...
	surface.DrawRect(0,0,ScrW()-(timeex2*ScrW()/10),24)
	surface.SetDrawColor(255,0,0,255)
	surface.DrawRect(2,2,ScrW()-(timeex2*ScrW()/10)-4,20)
	surface.SetFont("EventText")
	local w,h = surface.GetTextSize("Next event in "..math.Clamp(math.Round(10-timeex2),0,10).." seconds: "..events.GetNextEvent().name)
	draw.SimpleText("Next event in "..math.Clamp(math.Round(10-timeex2),0,10).." seconds: "..events.GetNextEvent().name, "EventText", math.Clamp(ScrW()-(timeex2*ScrW()/10)-4,w,ScrW())+1, h/2+2, Color(0,0,0, 255),TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)
	draw.SimpleText("Next event in "..math.Clamp(math.Round(10-timeex2),0,10).." seconds: "..events.GetNextEvent().name, "EventText", math.Clamp(ScrW()-(timeex2*ScrW()/10)-4,w,ScrW()), h/2+1, Color(255,255,255, 255),TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)
	if(events.GetCurrentEvent() != nil and events.GetCurrentEvent().drawcurrent == true)then
